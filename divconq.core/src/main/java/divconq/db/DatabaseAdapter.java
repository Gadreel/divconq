...
// TODO rename to differentiate that this is dcDB not just any db
public class DatabaseAdapter {
	protected DatabaseInterface conn = null;
	protected DatabaseTask task = null;
	
	// don't call for general code...
	public DatabaseAdapter(DatabaseInterface conn, DatabaseTask task) {
		this.conn = conn;
		this.task = task;
	}
 ...
 . i (filter'="")&(^dcProg("wherefilter",filter)'="") x "s res=$$"_^dcProg("wherefilter",filter)_"()"
		...
		for (int i = 0; i < parts.length; i++) {
			...
			if (StringUtil.isNotEmpty(pfname)) {
				values.set(i, this.get(table, id, pfname, when, pformat, historical));
				continue;
			}
