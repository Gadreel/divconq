 ...
 *
 */
abstract public class WrappedFuncCallback<T> extends FuncCallback<T> {
	protected int state = 0;  // 1 = delivered, -1 = canceled
	protected OperationResult or = null;
	
	public WrappedFuncCallback(OperationResult or, DebugLevel loglevel) {
		super(loglevel);
		this.or = or;
	}
	
	public WrappedFuncCallback(OperationResult or) {
		super();
		this.or = or;
	}
	
	public WrappedFuncCallback(TaskRun run) {
		super(run);
		this.or = run;
	}
