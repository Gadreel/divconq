...
public class TextArea extends Div implements IFormInput {
	...
	public void parseElement(ViewInfo view, Nodes nodes, XElement xel) {
		...
		if (xel.hasAttribute("Name")) {
			...
			children.add(new w3.html.TextArea(
					new Attributes("rows", "4", "cols", "40", "id", this.inputid, "name", this.inputid, "data-mini", "true", 
							"placeholder", ph, "readonly", ro, "data-record", this.record, "data-field", this.field), 
					view.getDomain().parseXml("HtmlOutput", view, xel), attrs
			));
			
			this.props = new RecordStruct(
					new FieldStruct("Type", "TextArea"), 
