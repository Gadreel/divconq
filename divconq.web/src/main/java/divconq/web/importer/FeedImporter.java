...
public class FeedImporter extends FileImporter {
	...
	public void preCheck(ImportWebsiteTool util) {
		...
		if ((this.pubfeed != null) && (this.prefeed != null)) {
			String pubuuid = this.pubfeed.getAttribute("Uuid");
			String preuuid = this.prefeed.getAttribute("Uuid");
			
			if (!pubuuid.equals(preuuid)) 
				OperationContext.get().error("Uuid does not match: " + this.key + " | " + this.pubfile + " | " + this.prefile);
		}
	...
	public void doImport(ImportWebsiteTool util, CountDownCallback cd) {
		...
		RecordStruct feed = new RecordStruct()
			.withField("Uuid", (this.pubfeed != null) ? this.pubfeed.getAttribute("Uuid") : this.prefeed.getAttribute("Uuid"))
